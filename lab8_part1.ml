...
module MakeInterval (Endpoint : ORDERED_TYPE) =
  struct
    ...
    let create (low : Endpoint.t) (high : Endpoint.t) : interval =
      ...
      (* Compare function better because is flexible with datatypes??
    ...
    let is_empty (intvl : interval) : bool =
      match intvl with
      | Empty -> true
      | Interval _ -> false (*don't need to compare again b/c already validated by create*)
...
let intvl2 = IntInterval.Interval (2,5) ;;
...
struct
  type endpoint = Endpoint.t (*NOTE: Why do we need this here?*)
  type interval =
    | Interval of Endpoint.t * Endpoint.t
    | Empty
